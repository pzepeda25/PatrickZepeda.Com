...
// ⚡ Bolt Optimization: Memoize the Three.js scanner component to prevent unnecessary re-renders when the parent's scroll state changes.
export const ScannerCardStream = memo(
  ({
    initialSpeed = 100,
    direction = -1,
    cardsData,
    ...
    useEffect(() => {
      cardStreamState.current.cardLineWidth =
    ...
    useEffect(() => {
      ...
      const updateCardEffects = () => {
        ...
        cardLine.querySelectorAll(".card-wrapper").forEach((wrapper, index) => {
          ...
          const asciiCard = wrapperEl.querySelector(
            ".card-ascii",
          ) as HTMLElement;
      ...
      const handleMouseUp = () => {
        ...
        if (Math.abs(cardStreamState.current.dragVelocity) > 0) {
          cardStreamState.current.currentVelocity =
            cardStreamState.current.dragVelocity * 60;
        }
      };
      ...
      cardLine.addEventListener("touchstart", handleMouseDown, {
        passive: true,
      });
      ...
      const animate = (currentTime: number) => {
        ...
        if (!isPaused && !cardStreamState.current.isDragging) {
          ...
          cardStreamState.current.currentVelocity +=
            (targetVelocity - cardStreamState.current.currentVelocity) *
            (1 - friction);

          cardStreamState.current.position +=
            cardStreamState.current.currentVelocity * deltaTime;
          setSpeed(
            Math.abs(Math.round(cardStreamState.current.currentVelocity)),
          );
        }
      ...
      `}</style>
        ...
        `}
          style={{
            ...
            boxShadow:
              "-6px 0 14px rgba(0,255,255,0.45), -14px 0 28px rgba(0,255,255,0.18)",
          }}
        />
        ...
        <div className="absolute inset-x-0 h-[288px] flex items-center justify-center z-[15] min-w-0">
          ...
          >
            {cards.map((card, idx) => (
              <div
                key={`${card.id}-${idx}`}
                className="card-wrapper relative h-[288px] shrink-0"
              ...
              >
                ...
                >
                  ...
                  <div className="p-4 flex flex-col flex-1 justify-between bg-black/50 whitespace-normal">
                    <h3 className="text-white font-bold text-sm line-clamp-2 font-sans group-hover:text-synth-cyan transition-colors leading-snug">
                      {card.title}
                    </h3>
                    <div className="flex justify-between items-center mt-2">
                      ...
                      <span className="text-synth-magenta text-xs font-mono group-hover:text-white transition-colors">
                        READ &gt;
                      </span>
                    </div>
                  </div>
